...
def start():
    ...
    if not q:
        messagebox.showwarning("Uwaga", "Wpisz frazę."); return
    try:
        ensure_api_key()
    ...
    btn_start.config(state="disabled"); prog.start(10); status.set("Szukam…")
    ...
    def worker():
        try:
            ...
            if not data:
                root.after(0, lambda: (prog.stop(), btn_start.config(state="normal"), status.set("Brak wyników.")))
                return
            csv_path, xlsx_path = save_results(data, ts, out_root)
            root.after(0, lambda: done(csv_path, xlsx_path, len(data), out_root))
        except Exception as e:
            root.after(0, lambda: (prog.stop(), btn_start.config(state="normal"),
                                   messagebox.showerror("Błąd", str(e))))

    threading.Thread(target=worker, daemon=True).start()

def done(csv_path, xlsx_path, n, out_root):
    prog.stop(); btn_start.config(state="normal")
    status.set(f"OK — zapisano {n} rekordów.")
    try: open_folder(out_root / "excel")
    except: pass
...
# --- UI ---
root = tk.Tk()
root.title("GinioCrawler"); root.geometry("560x220")

# fraza
tk.Label(root, text="Fraza do wyszukania:").pack(anchor="w", padx=12, pady=(12,0))
entry_query = tk.Entry(root)
entry_query.pack(fill="x", padx=12, pady=6); entry_query.focus()
